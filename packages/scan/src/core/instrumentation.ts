      ...
      onCommitFiberRoot(_rendererID, root) {
        ...
        traverseRenderedFibers(
          ...
          (fiber: Fiber, phase: 'mount' | 'update' | 'unmount') => {
            ...
            // Headless aggregate write — populates Store.legacyReportData
            // unconditionally so MCP / CDP / DevTools-panel consumers can
            // read per-component render counts without requiring the user
            // to enter Inspect mode and pick a focused component.
            const aggregateName = getDisplayName(type);
            if (aggregateName) {
              const t = getTimings(fiber);
              const prev = Store.legacyReportData.get(aggregateName) as
                | (OldRenderData & { mounts?: number })
                | undefined;
              if (prev) {
                prev.count += 1;
